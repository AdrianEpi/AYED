...
namespace AED {
	...
	//	void insert_pos(sll_node_t<T>* n, int pos);
	};
...
/*
	template <class T>
	void sll_t<T>::insert_pos(sll_node_t<T>* n, int pos)
	{
		sll_node_t<T>* sentinel = head_;
		for (int i = 0; i < pos - 1; i++)
		sentinel->set_next(sentinel->get_next());
		insert_after(sentinel, n);
		//pos->set_next(n);
